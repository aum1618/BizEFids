...
import {ChevronDown, ChevronLeft, Eye, Phone} from 'lucide-react-native';
...
export default function Presenter({color, text, navigation}) {
  ...
  const handleLogin = () => {
    if (phone.length === 0 || password.length === 0) {
      setError(true);
      navigation.navigate(screens.profileSetup);
    } else {
    }
  };
  return (
    <Wrapper color={{background: color.secondary}}>
      ...
      <View style={{flex: 1, padding: SIZES.SCALE_20}}>
        ...
        <View style={{marginVertical: SIZES.SCALE_20}}>
          <PrimaryButton color={color} onPress={handleLogin} text={'Login'} />
        </View>
        <View
          style={{
            height: 1,
